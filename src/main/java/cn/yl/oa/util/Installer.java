...
public class Installer {
	...
	public void install() {
		...
		session.save(user);
		//保存权限数据
		Privilege menu = new Privilege("系统管理",null,null);
		Privilege menu1 = new Privilege("岗位管理","/role_list",menu);
		Privilege menu2 = new Privilege("部门管理","/department_list",menu);
		Privilege menu3 = new Privilege("用户管理","/user_list",menu);
		session.save(menu);
		session.save(menu1);
		session.save(menu2);
		session.save(menu3);
		...
		//------------------------
		session.save(new Privilege("网上交流",null,null));
		session.save(new Privilege("论坛管理","/formManage_list",menu));
		session.save(new Privilege("论坛","/forum_list",menu));
		
		//-----------------------
		session.save(new Privilege("审批流转",null,null));
		session.save(new Privilege("审批流程管理","/processDefinition",menu));
		session.save(new Privilege("申请模板管理","/template_list",menu));
		session.save(new Privilege("起草申请","/flow_templateList",menu));
		session.save(new Privilege("待我审批","/flow_myTaskList",menu));
		session.save(new Privilege("我的申请查询","/flow_myApplicationList",menu));
	}
